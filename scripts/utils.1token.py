    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            if itr == 0:
                modified_kwargs = {}
                modified_kwargs['attention_mask'] = model_kwargs['attention_mask']
                modified_kwargs['cache_position'] = model_kwargs['cache_position']
                modified_kwargs['use_cache'] = model_kwargs['use_cache']
                modified_kwargs['past_key_values'] = model_kwargs['past_key_values']

                OFF = 32

                modified_kwargs['attention_mask'] = model_kwargs['attention_mask'][:, :-(OFF)]
                modified_kwargs['cache_position'] = model_kwargs['cache_position'][:-(OFF)]
                for offset in range(OFF):
                    modified_inputs = input_ids[:, :-(OFF - offset)]
                    #attention_mask': tensor([[1, 1, 1,  ..., 1, 1, 1]], device='cuda:0'), 'use_cache': True, 'past_key_values': DynamicCache(), 'cache_position': tensor([    0,     1,     2,  ..., 15682, 15683, 15684], device='cuda:0')}
                    #modified_kwargs['attention_mask'] = model_kwargs['attention_mask'][:, :-(OFF - offset)]
                    #modified_kwargs['cache_position'] = model_kwargs['cache_position'][:-(OFF - offset)]



                    model_inputs = self.prepare_inputs_for_generation(modified_inputs, **modified_kwargs)
                    ...
                    modified_kwargs = self._update_model_kwargs_for_generation(
                        outputs,
                        modified_kwargs,
                        is_encoder_decoder=self.config.is_encoder_decoder,
                    )
                ...
                model_kwargs['cache_position'] = modified_kwargs['cache_position']
